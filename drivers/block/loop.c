...
static void loop_end_io_transfer(struct bio *bio, int err)
{
	...
	if (bio_rw(bio) == WRITE) {
		...
		bio_endio(origbio, test_bit(0, &merge->bi_flags) ? (int)merge->LOOP_COMPAT_BI_SIZE : -EIO);
...
static struct bio *loop_get_buffer(struct loop_device *lo, struct bio *orig_bio,
		struct bio **merge_ptr, int *flushPtr, int origHasData, int *lastVePtr)
{
...
#endif
{
	...
	struct loop_bio_extension *extension;
	int rw = bio_rw(old_bio), y, x, flushFlag = 0, origHasData, lastVec = 0;
	char *md;
	wait_queue_t waitq;
	...
	new_bio = loop_get_buffer(lo, old_bio, &merge, &flushFlag, origHasData, &lastVec);
	if (!new_bio) {
	...
	for (;;) {
		...
		if (lo->lo_flags & LO_FLAGS_DO_BMAP) {
			...
			if (!atomic_dec_and_test(&merge->bi_cnt))
				continue;
			xbio->bi_next = NULL;
			bio_endio(xbio, test_bit(0, &merge->bi_flags) ? (int)merge->LOOP_COMPAT_BI_SIZE : -EIO);
			loop_put_buffer(lo, merge, 1);
		}
