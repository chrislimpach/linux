...
void braswell_gpio_set_alt(int gpio, int alt)
{
...
EXPORT_SYMBOL_GPL(braswell_gpio_set_alt);

int braswell_gpio_get_alt(int gpio)
{
	struct gpio_bank_pnp *bank;
	struct chv_gpio *cg = NULL;
	void __iomem *reg;
...
EXPORT_SYMBOL_GPL(braswell_gpio_get_alt);
...
chv_gpio_pnp_probe(struct pnp_dev *pdev, const struct pnp_device_id *id)
{
	...
	for (i = 0; i < nbanks; i++) {
        ...
        if (strcmp(pdev->name, "GPO1")){
		printk("Not GPO1, skip %s!\n",pdev->name);
		ret=0;
		goto err;
	}
