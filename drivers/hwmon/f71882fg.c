...
#define SIO_REG_ACPI_CTRL_1	0xF4
#define SIO_REG_ACPI_CTRL_2	0xF5
	...
	*devattr, const char *buf, size_t count)
{
	...
	val = clamp_val(val, data->temp_high[nr] - 15,
			    data->temp_high[nr]);
...
#define HWM_VERSION "20130227"
#define HWM_MODULE_NAME "Thecus hwm2 driver"
...
static int thecus_proc_hwm_show(struct seq_file *m, void *v)
{
	...
	superio_enter(0x4e);	// 0x4E

	superio_select(0x4e, SIO_F71882FG_LD_PME);
	temp = superio_inb(0x4e, SIO_REG_ACPI_CTRL_1);
	superio_exit(0x4e);

	seq_printf(m,"SIO_REG_ACPI_CTRL_1: %X\n", temp );
                   ...
                   size_t length, loff_t *ppos)
{
	...
	if (length < PAGE_SIZE)
	    ...
	    superio_select(0x4e, SIO_F71882FG_LD_PME);
            if (val1==0){
                printk("ACPWR=%d,Off\n",val1);
		superio_outb(0x4e, SIO_REG_ACPI_CTRL_1, 0x26);
            }else if (val1==1){
                printk("ACPWR=%d,On\n",val1);
		superio_outb(0x4e, SIO_REG_ACPI_CTRL_1, 0x24);
            }else{
                printk("ACPWR=%d,Keep last state\n",val1);
		superio_outb(0x4e, SIO_REG_ACPI_CTRL_1, 0x20);
            }
	    superio_exit(0x4e);
          }
        }
...
int thecus_hwm_init_procfs(void)
{
	struct proc_dir_entry *pde;
        ...
        pde = proc_create("hwm2", 0, NULL, &thecus_proc_hwm_operations);
        if (pde == NULL) {
                return -ENOMEM;
        }
...
static int __init f71882fg_init(void)
{
	...
	if( thecus_hwm_init_procfs()){
		printk(KERN_ERR "%s: cannot create /proc/hwm2 .\n", HWM_DRIVER_NAME);
		return -ENOENT;
	} else {
		printk(KERN_INFO "%s Loaded on /proc/hwm2 .\n", HWM_DRIVER_NAME);
		thecus_hwm_set_init();
	}
#endif
