 ...
 *
 * Driver for Fintek F75387SG2 chip on Thecus 1U4800R/S FAN3/FAN4
 */
...
MODULE_DESCRIPTION("Fintek F75387SG2 Driver");
MODULE_LICENSE("GPL");
static int debug;;
module_param(debug, int, S_IRUGO | S_IWUSR);

struct f75387sg2_data {
...
#define F75387SG_I2C_ID 0x2e
/* Addresses to scan */
static const unsigned short f75387sg2_addr[] = {
...
static int f75387sg2_rw(u8 reg_num, u8 * val, int wr)
{
    int ret = 0;
    int i, try_time = I2C_RETRY;

    if (NULL == save_client) {
	printk(KERN_INFO "f75387sg2: i2c_client is NULL\n");
	return 1;
    }
	...
	if (ret < 0) {
	    printk(KERN_INFO
		   "F75387SG2: cant write F75387SG2 Reg#0x%02X value=%d error (%d)\n",
		   reg_num, *val, ret);
	    return 1;
	}
    } else {			//Read
	if (ret < 0) {
	    printk(KERN_INFO
		   "F75387SG2: cant read F75387SG2 Reg#0x%02X error (%d)\n",
		   reg_num, ret);
	    return 1;
	}
    }
...
EXPORT_SYMBOL(f75387sg2_rw);

static int f75387sg2_probe(struct i2c_client *client,const struct i2c_device_id *id)
{
    struct f75387sg2_data *data;
    int ret = 0;
    u8 val1, val2;

    data = kzalloc(sizeof(struct f75387sg2_data), GFP_KERNEL);
    if (!data) {
	ret = -ENOMEM;
	goto exit_free;
    }
    ...
    printk("Attaching F75387SG2\n");

    val1 = 0;
    ret = f75387sg2_rw(0x5D, &val1, 0);
    if (ret > 0) {
	printk(KERN_INFO "f75387sg2_attach: cant read ctrl\n");
    }
    data->ctrl = val1;
    _DBG(1, "F75387SG2 Vendor ID=%02x\n", val1);

    val1 = 0;
    val2 = 0;
    f75387sg2_rw(0x5A, &val1, 0);
    f75387sg2_rw(0x5B, &val2, 0);
    chip_id = (val1 << 8) | val2;
    printk(KERN_INFO "f753xx: Chip id: %X\n", chip_id);

    if ((chip_id == F75375_CHIP_ID) || (chip_id == F75387_CHIP_ID)) {
      //f75387sg2_rw(0x1, &val1, 0);
      val1 = 0x4F;		// VT1,VT2 is connected to a BJT
      f75387sg2_rw(0x1, &val1, 1);
      f75387sg2_rw(0x1, &val1, 0);
      printk(KERN_INFO "reg 0x1: %X\n", val1);

      /* Set to manual mode. */
      val1 = 0x55;
      f75387sg2_rw(0x60, &val1, 1);
  ...
  exit_free:
    printk("f75387sg2_attach: fail\n");
    kfree(data);
    return ret;
}

static int f75387sg2_remove(struct i2c_client *client)
{
    struct f75387sg2_data *data = i2c_get_clientdata(client);
    kfree(data);
    save_client = NULL;
...
static ssize_t proc_f75387sg2_write(struct file *file,
				  ...
				  loff_t * ppos)
{
	...
	if (chip_id == F75375_CHIP_ID) {
	    ...
	    val1 = 0xF0;
	    f75387sg2_rw(val1, &val2, 0);
	    val2 = 0x2;
	    _DBG(1, "Set F75387SG2 register 0x%02X value to 0x%02X  \n", val1,
		 val2);
	    f75387sg2_rw(val1, &val2, 1);
	}
	f75387sg2_rw(0x1, &val1, 0);
...
static int proc_f75387sg2_show(struct seq_file *m, void *v)
{
    ...
    if (NULL == save_client) {
	seq_printf(m, "F75387SG2 not found\n");
	return 0;
    }

    if (!f75387sg2_rw(0x04, &val1, 0)) {
	seq_printf(m, "Address: %02X\n", val1);
    }

    if (!f75387sg2_rw(0x5c, &val1, 0)) {
    ...
    if (!f75387sg2_rw(0x14, &val1, 0)) {
	seq_printf(m, "Temp 1: %d\n", val1);
	if (chip_id == F75387_CHIP_ID)
	    f75387sg2_rw(0x1A, &val1, 0);
    }
    if (!f75387sg2_rw(0x15, &val1, 0)) {
	seq_printf(m, "Temp 2: %d\n", val1);
	if (chip_id == F75387_CHIP_ID)
	    f75387sg2_rw(0x1B, &val1, 0);
    }
    if (chip_id == F75387_CHIP_ID) {
	if (!f75387sg2_rw(0x1C, &val1, 0)) {
	    f75387sg2_rw(0x1D, &val2, 0);
	    seq_printf(m, "Temp 3: %d\n", val1);
	}
    }

    if (!f75387sg2_rw(0x74, &val1, 0)) {
	seq_printf(m, "FAN 1 Expected counter(MSB): %02X\n", val1);
    }
    if (!f75387sg2_rw(0x75, &val1, 0)) {
	seq_printf(m, "FAN 1 Expected counter(LSB): %02X\n", val1);
    }

    if (!f75387sg2_rw(0x84, &val1, 0)) {
	seq_printf(m, "FAN 2 Expected counter(MSB): %02X\n", val1);
    }
    if (!f75387sg2_rw(0x85, &val1, 0)) {
	seq_printf(m, "FAN 2 Expected counter(LSB): %02X\n", val1);
    }

    if (chip_id == F75375_CHIP_ID) {
	if (!f75387sg2_rw(0x69, &val1, 0)) {
	    seq_printf(m, "PWM 1 Raise duty: %02X\n", val1);
	}
	if (!f75387sg2_rw(0x6b, &val1, 0)) {
	    seq_printf(m, "PWM 1 Drop duty: %02X\n", val1);
	}
	if (!f75387sg2_rw(0x76, &val1, 0)) {
	    seq_printf(m, "PWM 1 duty: %02X\n", val1);
	}
	if (!f75387sg2_rw(0x60, &val1, 0)) {
	    seq_printf(m, "Reset timer control: %02X\n", val1);
	}
	if (!f75387sg2_rw(0x1, &val1, 0)) {
	    if (val1 & 0x10)
    ...
    if ((!f75387sg2_rw(0x16, &val1, 0))
	&& (!f75387sg2_rw(0x17, &val2, 0))) {
	i = val1;
	i = ((i << 8) & 0xff00) | val2;
	i *= pulse;
	if (i == 0)
    ...
    if ((!f75387sg2_rw(0x18, &val1, 0))
	&& (!f75387sg2_rw(0x19, &val2, 0))) {
	i = val1;
	i = ((i << 8) & 0xff00) | val2;
	i *= pulse;
	if (i == 0)
    ...
    if ((!f75387sg2_rw(0x70, &val1, 0))
	&& (!f75387sg2_rw(0x71, &val2, 0))) {
	i = val1;
	i = ((i << 8) & 0xff00) | val2;
	if (i == 0)
	    i = 1;		// avoid divide by zero
    ...
    if ((!f75387sg2_rw(0x80, &val1, 0))
	&& (!f75387sg2_rw(0x81, &val2, 0))) {
	i = val1;
	i = ((i << 8) & 0xff00) | val2;
	if (i == 0)
	    i = 1;		// avoid divide by zero
...
static struct file_operations proc_f75387sg2_operations = {
...
static const struct i2c_device_id f75387sg2_id[] = {
    {"F75387SG2", 0},
    {}
};

MODULE_DEVICE_TABLE(i2c, f75387sg2_id);

/* Return 0 if detection is successful, -ENODEV otherwise */
static int f75387sg2_detect(struct i2c_client *client,
                          struct i2c_board_info *info)
{
    struct i2c_adapter *adapter = client->adapter;
    unsigned short address = client->addr;
    const char *chip_name = "F75387SG2";
...
static struct i2c_driver f75387sg2_driver = {
    .driver = {
	       .name = "F75387SG2",
	       },
    .probe = f75387sg2_probe,
    .remove = f75387sg2_remove,
    .id_table = f75387sg2_id,

    .class = I2C_CLASS_HWMON,
    .detect = f75387sg2_detect,
    .address_list = f75387sg2_addr,
};


static __init int f75387sg2_init(void)
{
    struct proc_dir_entry *pde;

    pde = proc_create("f75387sg2", 0, NULL, &proc_f75387sg2_operations);
    if (pde == NULL) {
        printk(KERN_ERR "F75387SG2: cannot create /proc/f75387sg2.\n");
        return -ENOMEM;
    }

    return i2c_add_driver(&f75387sg2_driver);
}

static __exit void f75387sg2_exit(void)
{
    remove_proc_entry("f75387sg2", NULL);
    i2c_del_driver(&f75387sg2_driver);
}

module_init(f75387sg2_init);
module_exit(f75387sg2_exit);
