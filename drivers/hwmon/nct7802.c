                         ...
                         size_t count)
{
	...
	struct nct7802_data *data = dev_get_drvdata(dev);
        ...
        regmap_write(data->regmap, attr->index,
                           simple_strtoul(buf, NULL, 0));
        return count;
}

static ssize_t show_hex(struct device *dev,
...
/* 7.2.16... 0x10..0x13  Fan Count 1 Readout Register */
...
static SENSOR_DEVICE_ATTR(mode, S_IRUGO | S_IWUGO, show_hex, store_reg, REG_MODE);

/* 7.2.91... Fan Control Output Value */
static SENSOR_DEVICE_ATTR(pwm1, S_IRUGO | S_IWUGO, show_int, store_reg, 0x60);
static SENSOR_DEVICE_ATTR(pwm2, S_IRUGO | S_IWUGO, show_int, store_reg, 0x61);
static SENSOR_DEVICE_ATTR(pwm3, S_IRUGO | S_IWUGO, show_int, store_reg, 0x62);

/* 7.2.95... Temperature to Fan mapping Relationships Register */
static SENSOR_DEVICE_ATTR(temp_fan_map1, S_IRUGO | S_IWUGO, show_hex, store_reg, 0x64);
static SENSOR_DEVICE_ATTR(temp_fan_map2, S_IRUGO | S_IWUGO, show_hex, store_reg, 0x65);

/* 7.2.97... Fan Control Configuration Registers */
static SENSOR_DEVICE_ATTR(fan_ctl1, S_IRUGO | S_IWUGO, show_hex, store_reg, 0x66);
static SENSOR_DEVICE_ATTR(fan_ctl2, S_IRUGO | S_IWUGO, show_hex, store_reg, 0x67);
static SENSOR_DEVICE_ATTR(fan_ctl3, S_IRUGO | S_IWUGO, show_hex, store_reg, 0x68);

/* 7.2.108 Temp1 Hysteresis Register */
static SENSOR_DEVICE_ATTR(smartfan_hysteresis, S_IRUGO | S_IWUGO, show_hex, store_reg, 0x74);

/* 7.2.115... 0x80-0x83,0x84 Temperature (X-axis) transition */
static SENSOR_DEVICE_ATTR(smartfan_temp1, S_IRUGO | S_IWUGO, show_int, store_reg, 0x80);
static SENSOR_DEVICE_ATTR(smartfan_temp2, S_IRUGO | S_IWUGO, show_int, store_reg, 0x81);
static SENSOR_DEVICE_ATTR(smartfan_temp3, S_IRUGO | S_IWUGO, show_int, store_reg, 0x82);
static SENSOR_DEVICE_ATTR(smartfan_temp4, S_IRUGO | S_IWUGO, show_int, store_reg, 0x83);
static SENSOR_DEVICE_ATTR(smartfan_temp_crit, S_IRUGO | S_IWUGO, show_int, store_reg, 0x84);

/* 7.2.120... 0x85-0x88 PWM (Y-axis) transition */
static SENSOR_DEVICE_ATTR(smartfan_pwm1, S_IRUGO | S_IWUGO, show_int, store_reg, 0x85);
static SENSOR_DEVICE_ATTR(smartfan_pwm2, S_IRUGO | S_IWUGO, show_int, store_reg, 0x86);
static SENSOR_DEVICE_ATTR(smartfan_pwm3, S_IRUGO | S_IWUGO, show_int, store_reg, 0x87);
static SENSOR_DEVICE_ATTR(smartfan_pwm4, S_IRUGO | S_IWUGO, show_int, store_reg, 0x88);

static struct attribute *generic_attributes[] = {
...
static struct attribute *smartfan_attributes[] = {
	&sensor_dev_attr_temp_fan_map1.dev_attr.attr,
	&sensor_dev_attr_temp_fan_map2.dev_attr.attr,

	&sensor_dev_attr_fan_ctl1.dev_attr.attr,
	&sensor_dev_attr_fan_ctl2.dev_attr.attr,
	&sensor_dev_attr_fan_ctl3.dev_attr.attr,

	&sensor_dev_attr_smartfan_hysteresis.dev_attr.attr,

	&sensor_dev_attr_smartfan_temp1.dev_attr.attr,
	&sensor_dev_attr_smartfan_temp2.dev_attr.attr,
	&sensor_dev_attr_smartfan_temp3.dev_attr.attr,
	&sensor_dev_attr_smartfan_temp4.dev_attr.attr,
	&sensor_dev_attr_smartfan_temp_crit.dev_attr.attr,

	&sensor_dev_attr_smartfan_pwm1.dev_attr.attr,
	&sensor_dev_attr_smartfan_pwm2.dev_attr.attr,
	&sensor_dev_attr_smartfan_pwm3.dev_attr.attr,
	&sensor_dev_attr_smartfan_pwm4.dev_attr.attr,

	NULL
};

static const struct attribute_group *nct7802_groups[] = {
	&(const struct attribute_group){ .attrs = generic_attributes },
	&nct7802_temp_group,
	&nct7802_in_group,
	&nct7802_fan_group,
	&(const struct attribute_group){ .attrs = smartfan_attributes },
	NULL
};
...
struct nct7802_data *pdata = NULL;
#define THECUS_HWM_VER "0.01"
#define HWM_DRIVER_NAME "NCT7802"
#define THECUS_HWM 1
...
static void thecus_nct7802_init(struct i2c_client *client)
{
	printk ("thecus_nct7802_init\n");

}

static int thecus_proc_nct7802_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m,"Display %s Info Ver.%s\n", HWM_DRIVER_NAME, THECUS_HWM_VER);

	if(pdata == NULL) {
		seq_printf(m,"%s not find!\n", HWM_DRIVER_NAME);
		return 1;
	}

	seq_printf(m,"CPU_FAN RPM: %d\n", nct7802_read_fan(pdata, 0x10));
	seq_printf(m,"SYS_FAN1 RPM: %d\n", 0);
	seq_printf(m,"SYS_FAN2 RPM: %d\n", 0);
	seq_printf(m,"HDD_FAN1 RPM: %d\n", nct7802_read_fan(pdata, 0x11));
	seq_printf(m,"HDD_FAN2 RPM: %d\n", 0);
	seq_printf(m,"HDD_FAN3 RPM: %d\n", 0);
	seq_printf(m,"HDD_FAN4 RPM: %d\n", 0);

	err = nct7802_read_temp(pdata, 0x01, REG_TEMP_LSB, &temp);
	if (err < 0) return err;
	if (temp != 0) temp=temp/1000;
	seq_printf(m,"CPU_TEMP: %d\n", temp);

	err = nct7802_read_temp(pdata, 0x02, REG_TEMP_LSB, &temp);
	if (err < 0) return err;
	if (temp != 0) temp=temp/1000;
	...
	pm_con2 = readl(pmc_regmap + GEN_PMCON2);

	iounmap(pmc_regmap);

        seq_printf(m, "GEN_PMCON1: %x\n",pm_con1);
        seq_printf(m, "GEN_PMCON2: %x\n",pm_con2);

	return 0;
}

static int thecus_proc_nct7802_open(struct inode *inode, struct file *file)
{
        return single_open(file, thecus_proc_nct7802_show, NULL);
}

static ssize_t thecus_proc_nct7802_write(struct file *file, const char __user *buf,
                   size_t length, loff_t *ppos)
{
	...
	if (!strncmp(buffer, "ACPWR", strlen("ACPWR"))) {
		...
		if (i == 1){
			...
			if (val1==0){
				printk("ACPWR=%d,Off\n",val1);
				pm_con1_val = pm_con1 | 0x01;
				writel(pm_con1_val, pmc_regmap + GEN_PMCON1);
			}else if (val1==1){
				printk("ACPWR=%d,On\n",val1);
				pm_con1_val = pm_con1 & 0xFFFFFFFFFFFFFFFE;
				writel(pm_con1_val, pmc_regmap + GEN_PMCON1);
			}else{
...
static struct file_operations thecus_proc_nct7802_operations = {
        .open           = thecus_proc_nct7802_open,
        .read           = seq_read,
        .write          = thecus_proc_nct7802_write,
        .llseek         = seq_lseek,
        .release        = single_release,
};

static int thecus_nct7802_init_procfs(void)
{
        struct proc_dir_entry *pde;

        pde = proc_create("hwm", 0, NULL, &thecus_proc_nct7802_operations);
        if (pde == NULL) {
                return -ENOMEM;
        }
...
static void thecus_nct7802_exit_procfs(void)
{
