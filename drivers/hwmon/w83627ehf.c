...
{
	return clamp_val((mode ? (msec + 50) / 100 :
						(msec + 200) / 400), 1, 255);
}
...
{
	return clamp_val(((val + (scale_in[nr] / 2)) / scale_in[nr]), 0, 255);
}
			...
			const char *buf, size_t count)
{
	...
	struct sensor_device_attribute *sensor_attr = to_sensor_dev_attr(attr);
	int nr = sensor_attr->index;
	u32 val = clamp_val(simple_strtoul(buf, NULL, 10), 0, 255);
...
{\
	...
	struct sensor_device_attribute *sensor_attr = to_sensor_dev_attr(attr); \
	int nr = sensor_attr->index; \
	u32 val = clamp_val(simple_strtoul(buf, NULL, 10), 1, 255); \
	mutex_lock(&data->update_lock); \
	data->reg[nr] = val; \
				 ...
				 struct w83627ehf_sio_data *sio_data)
{
	...
	*addr = val & IOREGION_ALIGNMENT;
    ...
    //printk("THECUS DBG-1: CR30 %d\n",val);
	if (!(val & 0x01)) {
		printk(KERN_WARNING DRVNAME ": Forcibly enabling Super-I/O. "
		       "Sensor is probably unusable.\n");
		superio_outb(sioaddr, SIO_REG_ENABLE, val | 0x01);
	}
    //superio_outb(sioaddr, SIO_REG_ENABLE, 0);
    //val = superio_inb(sioaddr, SIO_REG_ENABLE);
    //printk("THECUS DBG-2: CR30 %d\n",val);
	superio_exit(sioaddr);
...
ChangeLog:
    ...
    Version 0.1.1: add /proc/hwm Control by Console.
    Version 0.1.2: change fan rpm count.
*/

#define THECUS_HWM_VER "0.1.3"
#if THECUS_HWM

/* Proc thecus_hwm */
#include <linux/proc_fs.h>
#include <linux/seq_file.h>
#define HWM_DRIVER_NAME "Thecus Hardware Monitor 0.1.1"


//Check for bit , return false when low
int thecus_check_bit(unsigned long val, int bn)
{
  if((val >> bn)&0x1){
...
static int thecus_proc_hwm_getrpm(u8 count,u8 div)
{
  int rpm=0;
  if(count==0)
    return 0;

  // RPM = (1.35 *1000000)/(Count * Divisor)
  rpm =1350000 / (count * (1<<div));
  if(rpm < 100)
    return 0;
  else
    return rpm;  
...
{
    /* Set Default Value of PC Health */
    u8 keep_fan_reg,fanmode_reg,sval;
    struct w83627ehf_data *data = w83627ehf_update_device(&pdev->dev);


    /// Set CPU FAN Default Value
    fanmode_reg = w83627ehf_read_value(data,0x04);

    // Set Bank0 04h bit 5,4 for CPUFAN to 11 as SMART FAN III MODE 
    //   to 00 Manual Mode, to 01 Thermal Cruise Mode, to 10 Fan Speed Cruise Mode
    sval=1;
    sval=sval<<4;
    fanmode_reg|=sval;
    //fanmode_reg &= ~sval;
    ...
    // Set CPU FAN Bank0 04h bit 1 to 0/1 as PWM/DC 
    sval=1;
    sval=sval<<1;
    //fanmode_reg&=~sval;
    fanmode_reg|=sval;

    w83627ehf_write_value(data,0x04,fanmode_reg);

    /// Set ALL FAN to Keep Minimum Fan Speed
    keep_fan_reg = w83627ehf_read_value(data,0x12);

    sval=1;
    sval=sval<<5; // SYSFAN2 Keep Min
    keep_fan_reg|=sval;

    sval=1;
    sval=sval<<4; // CPU FAN Keep Min
    keep_fan_reg|=sval;

    sval=1;
    sval=sval<<3; // SYSFAN1 Keep Min
    keep_fan_reg|=sval;

    w83627ehf_write_value(data,0x12,keep_fan_reg);

    // Other Default Value
    //w83627ehf_write_value(data,0x3c,0x01); /// CPU FAN MIN COUNT
    w83627ehf_write_value(data,0x09,0x7F); /// min_output fan speed == fan_stop_output
    w83627ehf_write_value(data,0x03,0xe7); /// init Fan Speed
    w83627ehf_write_value(data,0x67,0xff); /// Fan Max Reg
    w83627ehf_write_value(data,0x68,0x20); /// CPU Fan Step
    w83627ehf_write_value(data,0x06,0x28); /// CPU Fan Target TEMP.

    /// Set SYSFAN1 Default Value
    fanmode_reg = w83627ehf_read_value(data,0x12);

    // Set Bank0 12h bit 2,1 for SYSFAN1 to 00 as Manual MODE 
    //Bit:2-1, 00 Manual Mode, to 01 Thermal Cruise Mode, to 10 Fan Speed Cruise Mode
    ...
    // Set SYSFAN1 Bank0 12h bit 0 to 0/1 as PWM/DC 
    sval=1;
    //sval=sval<<1;
    //fanmode_reg&=~sval;
    fanmode_reg|=sval;

    w83627ehf_write_value(data,0x12,fanmode_reg);
    //w83627ehf_write_value(data,0x3d,0x01); /// SYSFAN1 FAN MIN COUNT
    w83627ehf_write_value(data,0x15,0x7F); /// min_output fan speed == fan_stop_output
    w83627ehf_write_value(data,0x11,0xe7); /// init Fan Speed
    w83627ehf_write_value(data,0x13,0x32); /// SYSFAN1 Target TEMP.

    /// Set SYSFAN2 Default Value
    fanmode_reg = w83627ehf_read_value(data,0x62);

    // Bit5-4: 00 Manual Mode, to 01 Thermal Cruise Mode, to 10 Fan Speed Cruise Mode, to 11 Smart Fan III Mode
    sval=1;
    sval=sval<<4;
    //fanmode_reg|=sval;
    fanmode_reg &= ~sval;
    ...
    // Set SYSFAN2 Bank0 62h bit 6 to 0/1 as PWM/DC 
    sval=1;
    sval=sval<<6;
    //fanmode_reg&=~sval;
    fanmode_reg|=sval;

    w83627ehf_write_value(data,0x62,fanmode_reg);
    //w83627ehf_write_value(data,0x3e,0x01); /// SYSFAN2 FAN MIN COUNT
    w83627ehf_write_value(data,0x64,0x7F); /// min_output fan speed = fan_stop_output
    w83627ehf_write_value(data,0x61,0xe7); /// init Fan Speed
    w83627ehf_write_value(data,0x69,0xff); /// SYSFAN2 Max Reg
    w83627ehf_write_value(data,0x6A,0x20); /// SYSFAN2 Fan Step
    w83627ehf_write_value(data,0x63,0x32); /// SYSFAN2 Target TEMP.

    /// Disable CPUTIN Limit Beep, useless.
    fanmode_reg = w83627ehf_read_value(data,0x56);
    sval=1;
    sval=sval<<5;
    fanmode_reg&=~sval;

    w83627ehf_write_value(data,0x56,fanmode_reg);

    /// Set OVT POL Low
    fanmode_reg = w83627ehf_read_value(data,0x4c);
    sval=1;
    sval=sval<<2;
    fanmode_reg&=~sval;

    w83627ehf_write_value(data,0x4c,fanmode_reg);
...
{
    int rpm=0,i=0;
    s8 cputemp=0,systemp=0,hddtemp=0;
    u8 cpu_ovt;
    int rpm0=0;

    struct w83627ehf_data *data = w83627ehf_update_device(&pdev->dev);
    seq_printf(m,"Display Winbond w83627ehg Info Ver.%s--- \n",THECUS_HWM_VER);

    /// Thecus: We Don't care the NBFAN Speed. 
    rpm0=thecus_proc_hwm_getrpm(data->fan[0],data->fan_div[0]);
    //    seq_printf(m,"FAN 0 RPM: %d\n",rpm0); //NBFAN

    rpm=thecus_proc_hwm_getrpm(data->fan[1],data->fan_div[1]);
    seq_printf(m,"CPU_FAN RPM: %d\n",rpm); //CPUFAN1

    rpm=thecus_proc_hwm_getrpm(data->fan[2],data->fan_div[2]);
    seq_printf(m,"HDD_FAN1 RPM: %d\n",rpm); //SYSFAN1

    rpm=thecus_proc_hwm_getrpm(data->fan[3],data->fan_div[3]);
    if (rpm>0) seq_printf(m,"HDD_FAN2 RPM: %d\n",rpm); //SYSFAN2
    else {
    	//Get RPM from NB FAN
    	seq_printf(m,"HDD_FAN2 RPM: %d\n",rpm0); //SYSFAN2
    }

    cputemp=data->temp[0]>>8;
    seq_printf(m,"CPU_TEMP: %d\n",cputemp);

    systemp=(int)data->temp1;
    seq_printf(m,"SAS_TEMP: %d\n",systemp);

    hddtemp=data->temp[1]>>8;
                   ...
                   size_t length, loff_t *ppos)
{
    ...
    /*
     * Set Fan MODE
     * Usage: echo "AUTO FAN1~3 0|1|2|3 $PARAM" >/proc/thecus_hwm
     *   ex. AUTO FAN1 0 0~255 --> Manual Mode,Output Voltage=AVCC*((input value/4)/64)
     *   ex. AUTO FAN1 1 30 --> Thermal Mode Target,Set Target Temp as 30C
     *   ex. AUTO FAN1 2 2000 --> Fan Speed Mode,Set RPM as 2000
     *   ex. AUTO FAN1 3 30 --> SMART.III Mode Target,Set Target Temp as 30C
     *
     *
     */
    

    if (!strncmp (buffer, "AUTO", strlen ("AUTO"))){
        i = sscanf (buffer + strlen ("AUTO FAN"), "%d %d %d\n",&fan,&val1,&val2);
        printk("CMD: %s %d,MODE:%d VALUE:%d\n","Auto FAN", fan,val1,val2);
        if (i==3 && val1 < 4){ // 3 inputs
            if(fan==2 && val1 ==3){
                printk("THECUS_HWM:FAN2 Doesn't Support Smart Fan III Mode!!\n");
            }else{
                mutex_lock(&data->update_lock);
                /// Setting FAN MODE
                reg = w83627ehf_read_value(data, W83627EHF_REG_PWM_ENABLE[fan]);
                data->pwm_enable[fan] = val1;
                reg &= ~(0x03 << W83627EHF_PWM_ENABLE_SHIFT[fan]);
                reg |= (val1) << W83627EHF_PWM_ENABLE_SHIFT[fan];

                /// Setting FAN MODE PARAMETER
                switch (val1) {
                case 0:  
                    if(val2 >= 0 && val2<=255){
                      w83627ehf_write_value(data, W83627EHF_REG_PWM_ENABLE[fan], reg);
                      data->pwm[fan]=val2;
                      w83627ehf_write_value(data, W83627EHF_REG_PWM[fan], val2);
                    }
                    else{
                      printk("THECUS_HWM:Manual Fan Mode only Support FAN Voltage Level:0~255.\n");
                    }
                    break;
                case 1:
                    w83627ehf_write_value(data, W83627EHF_REG_PWM_ENABLE[fan], reg);
                    data->target_temp[fan] = val2;
                    w83627ehf_write_value(data, W83627EHF_REG_TARGET[fan], val2);
                    break;
                case 3:
                case 2:
                    //if(val2 > 0 && val2 <255){
                    //w83627ehf_write_value(data, W83627EHF_REG_PWM_ENABLE[fan], reg);
                    //w83627ehf_write_fan_div(data,fan);
                    // count=1350000 / (RPM * FAN_DIV)
                    //div = 1<<data->fan_div[fan];
                    //count=1350000/(val2 * (2>>div));
                    //count = val2;
                    //data->target_temp[fan] = count;
                    //w83627ehf_write_value(data, W83627EHF_REG_TARGET[fan], count);
                    //}
                    break;
                }
...
static int __init sensors_w83627ehf_init(void)
{
    ...
    if( thecus_hwm_init_procfs()){
      printk(KERN_ERR "MV_GPIO: cannot create /proc/hwm .\n");
      return -ENOENT;
    }   else{
