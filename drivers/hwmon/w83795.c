...
*/
	...
	seq_printf(m,"SYS_TEMP: %d\n", data->temp[1][TEMP_READ]);
        ...
        if ( fan_type == 1 ){
	    /* N7710/N7770 show temp1 only , temp2 should always show -128 */
	    seq_printf(m,"HDD_TEMP1: %d\n", data->temp[2][TEMP_READ]);
        } else if ( fan_type == 2 ){
	    /* N8810/N8880 need switch hdd temp1/temp2, temp1 should always show -128 */
	    seq_printf(m,"HDD_TEMP1: %d\n", data->temp[3][TEMP_READ]);
	} else if ((data->temp[2][TEMP_READ] == -128) && (data->temp[3][TEMP_READ] > -128)){
            /* Smart switch hdd temp1/temp2, when detect -128 */
            /* Kepp this for old f/w */
	    seq_printf(m,"HDD_TEMP1: %d\n", data->temp[3][TEMP_READ]);
