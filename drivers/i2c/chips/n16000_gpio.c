...
static int pch_gpio_probe(struct pci_dev *, const struct pci_device_id *);
...
static int pch_gpio_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
    int ret;
...
int PCH_GPIO_init(void)
{
    struct pci_dev *pdev = NULL;
    const struct pci_device_id *ent = NULL;

    for_each_pci_dev(pdev) {
      ent = pci_match_id(pch_gpio_pci_tbl, pdev);
      if (ent) break;
    }
