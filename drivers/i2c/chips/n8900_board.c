...
u32 n7710_disk_access(int index, int act);
u32 n7710_disk_index(int index, struct scsi_device *sdp);
...
static const struct thecus_function n7710_func = {
        .disk_access = n7710_disk_access,
        .disk_index  = n7710_disk_index,
};
...
static struct thecus_board board_info [] = {
	...
	{ 9 , "N12000V"  , 12, 17, 2, "611", "BOARD_N8900", &n8900_func},
        { 10, "N7710"    , 7 , 17, 0, "612", "BOARD_N8900", &n7710_func},
        { 11, "N8810U"   , 8 , 17, 0, "613", "BOARD_N8900", &n7710_func},
        { 12, "N7710-G"  , 7 , 17, 0, "614", "BOARD_N8900", &n7710_func},
        { 13, "N8810U-G" , 8 , 17, 0, "615", "BOARD_N8900", &n7710_func},
        { 14, "N7770"    , 7 , 17, 0, "616", "BOARD_N8900", &n7710_func},
        { 15, "N8880U"   , 8 , 17, 0, "617", "BOARD_N8900", &n7710_func},
        { 16, "N7770-10G"  , 7 , 17, 0, "618", "BOARD_N8900", &n7710_func},
        { 17, "N8880U-10G" , 8 , 17, 0, "619", "BOARD_N8900", &n7710_func},
        { 18 ,"N6850PLUS"  , 6 , 17, 1, "620", "BOARD_N8900", &n6850_func},
	{ }
};
...
u32 n7710_disk_index(int index, struct scsi_device *sdp){
    ...
    if(0==strncmp(sdp->host->hostt->name,"usb-storage",strlen("usb-storage"))){
      if(0==strncmp(sdp->host->hostt->info(sdp->host),"usb-storage 2-1.1:1.0",strlen("usb-storage 2-1.1:1.0"))){
        sdp->disk_if = 0;      // Fake SATA
        tindex = 702;
      }
    }else if(0==strncmp(sdp->host->hostt->name,"sata_sil24",strlen("sata_sil24"))){
        tindex = sdp->host->host_no - 6;
    }else if(0==strncmp(sdp->host->hostt->name,"ahci",strlen("ahci"))){
        tindex = sdp->host->host_no; // For SATA DOM
        switch (tindex) {
            case 0: 
		tindex = 702;
                break;
...
static int proc_thecus_io_show(struct seq_file *m, void *v)
{
    ...
    if ((board_idx < 4) || ((board_idx > 6) && (board_idx != 18))){
