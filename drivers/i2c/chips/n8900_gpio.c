 ...
 *
 *      Driver for 6 series and C200 series GPIO on Thecus N8900 Board
 *
...
#define NAME	"pch_6_gpio"

MODULE_AUTHOR("Citizen Lee");
MODULE_DESCRIPTION("Intel 6 series and C200 series GPIO Driver for Thecus N8900 Board");
MODULE_LICENSE("GPL");

/* Module and version information */
#define GPIO_VERSION "20110427"
#define GPIO_MODULE_NAME "Intel 6 series and C200 series GPIO driver"
...
static struct pci_dev *pch_6_gpio_pci = NULL;
...
-1: Error
*/
int PCH_6_gpio_read_bit(u32 bit_n)
{
    int ret = 0;
    u32 gval = 0;
...
-1: Error
*/
int PCH_6_gpio_write_bit(u32 bit_n, int val)
{
    int ret = 0;
 ...
 */
static struct pci_device_id pch_6_gpio_pci_tbl[] = {
    /*{PCI_DEVICE(PCI_VENDOR_ID_INTEL, 0x1c4e)}, *//* Q67 */
    {PCI_DEVICE(PCI_VENDOR_ID_INTEL, 0x1c56)}, /* C200 */
    {0,},			/* End of list */
};

MODULE_DEVICE_TABLE(pci, pch_6_gpio_pci_tbl);

static int pch_6_gpio_probe(struct pci_dev *, const struct pci_device_id *);

static struct pci_driver pch_6_gpio_pci_driver = {
        .name = "pch_6_gpio",
        .id_table = pch_6_gpio_pci_tbl,
        .probe = pch_6_gpio_probe,
};

/*
 *	Init & exit routines
 */
static unsigned char __init pch_6_gpio_getdevice(struct pci_dev *dev)
{
    u8 val1, val2;
    u32 badr;
    u32 gval, sval;
	...
	pci_read_config_byte(pch_6_gpio_pci, GPIO_CNTL, &val1);
	if (val1 == 0x10) {
	    //pci_write_config_byte (pch_6_gpio_pci, GPIO_CNTL, 0);
	    printk(KERN_INFO NAME ": GPIO already turned on\n");
	} else {
	    pci_write_config_byte(pch_6_gpio_pci, GPIO_CNTL, 0x10);
	    printk(KERN_INFO NAME ": Turn on the GPIO\n");
	}

        // get PM_ADDR
	pci_read_config_byte(pch_6_gpio_pci, PMBASE, &val1);
	pci_read_config_byte(pch_6_gpio_pci, PMBASE + 1, &val2);
	badr = ((val2 << 1) | (val1 >> 7)) << 7;	// 15:7
	...
	/* GPIO SEL */
	// Used GPIO 2,3,21,22,31,38,39,48,49,57,72
	...
	// GPIO 2,21,22,31,38,39,48,49,57,72 Should be as Input = 0
...
/*
	pci_read_config_byte(pch_6_gpio_pci, GPIO_PMCON_2, &val1);
	pci_read_config_byte(pch_6_gpio_pci, GPIO_PMCON_3, &val2);
	printk("(1)PMCON_2=0x%02X\n   PMCON_3=0x%02X\n", val1, val2);
	val2 = 0x0;
	pci_write_config_byte(pch_6_gpio_pci, GPIO_PMCON_3, val2);
	pci_read_config_byte(pch_6_gpio_pci, GPIO_PMCON_2, &val1);
	pci_read_config_byte(pch_6_gpio_pci, GPIO_PMCON_3, &val2);
...
static int pch_6_gpio_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
    int ret;

    /* Check whether or not the PCH LPC is there */
    if (!pch_6_gpio_getdevice(pdev) || pch_6_gpio_pci == NULL)
	return -ENODEV;
...
int PCH_6_GPIO_init(void)
{
    struct pci_dev *pdev = NULL;
    const struct pci_device_id *ent = NULL;

    for_each_pci_dev(pdev) {
      ent = pci_match_id(pch_6_gpio_pci_tbl, pdev);
      if (ent) break;
    }
    ...
    return pci_register_driver(&pch_6_gpio_pci_driver);
}

void PCH_6_GPIO_exit(void)
{
    // Deregister
    pci_unregister_driver(&pch_6_gpio_pci_driver);
    if (GPIO_ADDR != 0) release_region(GPIO_ADDR, GPIO_IO_PORTS);
}

/*
static int __init PCH_6_GPIO_init(void)
{
    printk(KERN_INFO NAME ": %s\n", GPIO_DRIVER_NAME);
...
static void __exit PCH_6_GPIO_exit(void)
{
    // Deregister
    pci_unregister_driver(&pch_6_gpio_pci_driver);
    if (GPIO_ADDR != 0) release_region(GPIO_ADDR, GPIO_IO_PORTS);
...
module_init(PCH_6_GPIO_init);
module_exit(PCH_6_GPIO_exit);
*/
