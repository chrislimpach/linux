...
void check_raid_status(struct mddev * mddev, int status)
{
    int i;
					   ...
					   size_t length, loff_t * ppos)
{
	...
	if (buffer) {
		sscanf(buffer, "%s %s\n", item, target);
		if (!strncmp(item, "raid_damage", strlen("raid_damage"))) {
			criticalevent_user(RAID_DAMAGE, target);
		} else if (!strncmp(item, "raid_disk_fail", strlen("raid_disk_fail"))) {
			criticalevent_user(RAID_DISK_FAIL, target);
		} else {
...
static __init int thecus_event_init(void)
{
    ...
    cpde = proc_create("thecus_eventc", S_IRUSR, NULL, &proc_thecus_event_critical_operations);
    if (cpde == NULL) {
        return -ENOMEM;
    }
