 ...
 * Copyright (C) 2011 Tehuti Networks Ltd.. All rights reserved
 *
 ...
 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 * There are 2 types of RX communication channels between the driver and the
 * NIC.
 * 1) RX Free Fifo - RXF holds descriptors of empty buffers to accept incoming
 *    traffic. This fifo is filled by SW and is read by HW. Each descriptor
 *    holds information about the buffer's location, size and ID. The ID field
 *    is used to identify a buffer when it is returned with data via RXD Fifo
 *    (see below).
 *
 * 2) RX Data Fifo - RXD holds descriptors of full buffers. This fifo is
 *    filled by HW and is read by SW. Each descriptor holds status and ID.
 *    HW pops descriptor from the RXF fifo, stores the ID, fills the buffer with
 *    incoming data , moves it into host memory via DMA, builds new RXD
 *    descriptor with same ID, and pushes it into RXD Fifo and raises
 *    interrupt to indicate new RX data.
 *
 * The current NIC configuration (registers + firmware) uses 2 RXF fifos.
 * One holds 1.5K packets and the other 26K packets. Depending on the incoming
 * packet size, the HW decides which RXF Fifo to pop the buffer from. When a
 * packet is filled with data, the HW builds new RXD descriptor for it and
 * pushes it into a single RXD Fifo.
 *
 *
 * RX SW Data Structures
 * ~~~~~~~~~~~~~~~~~~~~~
 * The skb db is used to keep track of all skbs owned by SW and their DMA
 * addresses. For RX case, ownership lasts from empty skb allocation to the
 * RXF till the received skb is accepted from RXD and passed to the OS. Each
 * RXF Fifo has its own skb db.
 *
 * Fifo objects keep information about the fifo size and location, relevant HW
 * registers, usage and skb db. Each RXD and RXF fifo has its own fifo
 * structure. Implemented as simple struct.
 *
 *
 * RX SW Execution Flow
 * ~~~~~~~~~~~~~~~~~~~~
 * Upon initialization (ifconfig up) the driver creates RX fifos and
 * initializes the relevant registers. At the end of init phase, the driver
 * enables interrupts. The NIC sees that there are no RXF buffers and raises
 * RD_INTR interrupt, The ISR fills skbs and Rx begins.
 *
 * The driver has two receive operation modes:
 * - NAPI interrupt-driven mixed with polling
 * - Interrupt-driven only
 *
 * In Interrupt-driven only flow // when buffer is ready, the HW raises an
 * interrupt and the ISR is called. The ISR collects all available packets
 * (bdx_rx_receive), refills skbs (bdx_rx_alloc_buffers) and exit.
 *
 * Rx buffer allocation note
 * ~~~~~~~~~~~~~~~~~~~~~~~~~
 * The driver cares to feed such amount of RxF descriptors that respective
 * amount of RxD descriptors can not fill entire RxD fifo. The main reason for
 * that is lack of overflow check in Bordeaux for RxD fifo free/used size.
 *
...
MODULE_PARM_DESC(paged_buffers,
		 "When 0 use skbs otherwise allocate paged buffers");
#else
const uint paged_buffers = 0;
#endif
//uint hw_rx_hash	   = 1;
// module_param(hw_rx_hash, uint, 0644);
// MODULE_PARM_DESC(hw_rx_hash, "When 1 generate rxhash in hardware");

static u32 hashrnd __read_mostly;

#define LDEV(_vid,_pid,_subdev,_msi,_ports,_phya,_phyb,_name)  \
	{_vid,_pid,_subdev,_msi,_ports,PHY_##_phya,PHY_##_phyb}

static struct bdx_device_descr  bdx_dev_tbl[] =
{
LDEV(TEHUTI_VID,0x3014,0x3014,1,2,CX4,CX4,"TN3016-D  CX4 "),
LDEV(TEHUTI_VID,0x3114,0x3014,0,2,CX4,CX4,"TN3020-D  CX4 dual port"),
LDEV(TEHUTI_VID,0x3314,0x3014,1,2,CX4,CX4,"TN3020-DB CX4 dual port"),       
LDEV(TEHUTI_VID,0x3010,0x3002,1,1,QT2025x,NA,"TN3016-D XFP single port"),   
LDEV(TEHUTI_VID,0x3014,0x3003,1,2,QT2025x,QT2025x,"TN3016-D XFP dual port"),
LDEV(TEHUTI_VID,0x3010,0x3004,1,1,QT2025,NA,"TN3016-D SFP+ single port"),   
LDEV(TEHUTI_VID,0x3110,0x3004,0,1,QT2025,NA,"TN3020-D SFP+ single port"),   
LDEV(TEHUTI_VID,0x3310,0x3004,1,1,QT2025,NA,"TN3020-DB SFP+ single port"),  
LDEV(TEHUTI_VID,0x3014,0x3005,1,2,QT2025,QT2025,"TN3016-D SFP+  dual port"),    
LDEV(TEHUTI_VID,0x3114,0x3005,0,2,QT2025,QT2025,"TN3020-D SFP+ dual port"), 
LDEV(TEHUTI_VID,0x3314,0x3005,1,2,QT2025,QT2025,"TN3020-DB SFP+ dual port"),
LDEV(TEHUTI_VID,0x3114,0x3011,0,2,QT2025,CX4,"TN3020-D SFP+/CX4"),          
LDEV(TEHUTI_VID,0x3314,0x3011,1,2,QT2025,CX4,"TN3020-DB SFP+/CX4"),         
LDEV(TEHUTI_VID,0x3114,0x3012,0,2,CX4,QT2025,"TN3020-D  CX4/SFP+"),         
LDEV(TEHUTI_VID,0x3314,0x3012,1,2,CX4,QT2025,"TN3020-DB CX4/SFP+"),         
LDEV(TEHUTI_VID,0x3310,0x3015,1,1,TN8020,NA,"TN6001 10GbEbaseT single-Port"),
LDEV(TEHUTI_VID,0x3314,0x3016,1,2,TN8020,NA,"TN6002 10GbEbaseT dual port"),
{0}};
...
static struct pci_device_id bdx_pci_tbl[] = {
	PCI_REC(0x1FC9, 0x3014),/*clean SROM*/
	PCI_REC(TEHUTI_VID,0x3009),
	PCI_REC(TEHUTI_VID,0x3010),
	PCI_REC(TEHUTI_VID,0x3114),
	PCI_REC(TEHUTI_VID,0x3110),
	PCI_REC(TEHUTI_VID,0x3314),
	PCI_REC(TEHUTI_VID,0x3310),
{0}};
 ...
 * mdio_get).
 */
static inline u32 bdx_mdio_get(void __iomem *regs)
{
 ...
 */
static inline int bdx_mdio_read(void __iomem *regs, int device,
                                int port, u16 addr)
{
	u32 tmp_reg;
	/* Wait until MDIO is not busy */
	if (bdx_mdio_get(regs)== 0xFFFFFFFF) {
		ERR("MDIO busy - can't read\n");
		return -1;
	}
	/* Write address */
	writel((u32)addr, regs + regMDIO_ADDR);
	/* Write read command */
	writel(MDIO_CMD_STAT_VAL(1, device, port), regs + regMDIO_CMD_STAT);
	/* Read CMD_STAT until not busy */
	if ((tmp_reg = bdx_mdio_get(regs)) == 0xFFFFFFFF) {
		ERR("MDIO busy after read command\n");
		return -1;
	}
	if (GET_MDIO_RD_ERR(tmp_reg)) {
		//ERR("MDIO error after read command\n");
		return -1;
	}
 ...
 */
static inline int bdx_mdio_write(void __iomem *regs, int device, int port,
				 u16 addr, u16 data)
{
	u32 tmp_reg;
	...
	/* Write write command */
	writel(MDIO_CMD_STAT_VAL(0, device, port), regs + regMDIO_CMD_STAT);
	/* Read CMD_STAT until not busy */
	if ((tmp_reg = bdx_mdio_get(regs)) == 0xFFFFFFFF) {
...
#define PHY_MDIO_READ(priv,device,addr) bdx_mdio_read((priv)->pBdxRegs,(device),(priv)->phy_mdio_port,(addr))
#define PHY_MDIO_WRITE(priv,device,addr,data) bdx_mdio_write((priv)->pBdxRegs,(device),(priv)->phy_mdio_port,	 (addr), (data))


static int bdx_mdio_look_for_phy(void __iomem *regs, int port){
     int i=port, phy_id;
     msleep(100);
     phy_id= bdx_mdio_read(regs, 1, i, 0x0002); // PHY_ID_HIGH
     phy_id &=0xFFFF;
     if (phy_id!=0xFFFF && phy_id!= 0)     return i;
     for (i=0;i<32;i++){
        msleep(10);
        phy_id= bdx_mdio_read(regs, 1, i, 0x0002); // PHY_ID_HIGH
        phy_id &=0xFFFF;
...
#define	 BDX_MDIO_WRITE(space, addr, val) \
	if (bdx_mdio_write(regs, (space), port, (addr), (val))) {     \
...
static int QT2025_mdio_reset(void __iomem * regs, int port,
			  unsigned short phy)
{
	u16 *phy_fw= phy_firmware_a;
	int s_fw;
	int i, a, xfp= 0;
	u16 j;

	int phy_type, phy_type_b, phy_id= 0, rev= 0;

        port=bdx_mdio_look_for_phy(regs,port);
        if (port<0) return 0; // HPY not found
        
	switch (phy) {
	case PHY_QT2025x: /*  0x3002 0x3003 */
		phy_type = 0x13;
                phy_type_b= 0x90;
                xfp= 1;                  /* xfp */
		break;
	case PHY_QT2025: /* 0x3004 0x3005 0x3011 0x3012 */
		phy_type = 0x9b;
                phy_type_b= 0x98;        /* sfp+ */
		break;
	default:
		ERR("PHY_dev %d, returning\n", (int)phy);
		return 0;
	}
	...
	case 0xa1:
	case 0xa2: phy_fw = phy_firmware_a;
                rev = 0xa;
                break;
	case 0xb2:  //rev c
	case 0xb1: phy_fw= phy_firmware_b;
                rev= 0xb;
                s_fw= sizeof(phy_firmware_b) / sizeof(u16);
		break; //rev b
	case 0xb3: phy_fw= phy_firmware_d; rev= 0xd;
                s_fw= sizeof(phy_firmware_d) / sizeof(u16);
		break;
	default:
	...
	for (i = 0; i < sizeof(phy_firmware_a) / sizeof(u16); i++) {
                if (phy_firmware_a[i] < 0x100) {
                        BDX_MDIO_WRITE(3,(0x8000 + i), phy_firmware_a[i]);
                }
	}
	BDX_MDIO_WRITE(4, 0xC05B, 0x0012);
	BDX_MDIO_WRITE(1, 0xC302, 0x0044);
	BDX_MDIO_WRITE(1, 0xC319, 0x0001);
	BDX_MDIO_WRITE(1, 0xC31A, phy_type);
	BDX_MDIO_WRITE(3, 0xE854, 0x0040);

	msleep(10);

	BDX_MDIO_WRITE(1, 0xC30A, 0x00E6);
	BDX_MDIO_WRITE(1, 0xC309, 0x3804);
	BDX_MDIO_WRITE(1, 0xC308, 0x9313);
		break;
	...
	BDX_MDIO_WRITE(1, 0xC300, 0x0000);
	BDX_MDIO_WRITE(1, 0xC302,(rev== 0xd ? 0x4 : 0x000C));
	BDX_MDIO_WRITE(1, 0xC319, 0x0008);
	BDX_MDIO_WRITE(1, 0xC31A, phy_type_b);
	BDX_MDIO_WRITE(3, 0x0026, 0x0E00);
	BDX_MDIO_WRITE(3, 0x0027, 0x0000);
	...
	BDX_MDIO_WRITE(3, 0xE854, 0x00C0);
		...
		if(xfp) {
			u16 i;
                        ...
                        i  = bdx_mdio_read(regs, 1, port, 0xC308);
			i |= 1 << 9;
                        BDX_MDIO_WRITE(1, 0xC308, i);
                        i  = bdx_mdio_read(regs, 1, port, 0xC309);
			i |= 1 << 12;
                        BDX_MDIO_WRITE(1, 0xC309, i);
                        i  = bdx_mdio_read(regs, 1, port, 0xC30A);
                        i |= 3; i&=~(3 << 2);
                        BDX_MDIO_WRITE(1, 0xC30A, i);
                        i  = bdx_mdio_read(regs, 1, port, 0xC308);
			i &= ~(1 << 14);
                        BDX_MDIO_WRITE(1, 0xC308, i);		
                        msleep(10);
...
static int TN8020_mdio_reset(void __iomem * regs, int port, unsigned short phy){ 
	int i;
	int phy_id=0;
        u16 adHi,adLo;
...
// Load PHY FW
        {
         u32 pl;
         BDX_MDIO_WRITE(31,0x0000,3); // set PHY reset & auto-increment
         for(i=0;i<TN8020_phy_firmware_size;){
             // Set addr
             adHi=(u16) (TN8020_phy_firmware[i]>>16);
             adLo=(u16) TN8020_phy_firmware[i];
             BDX_MDIO_WRITE(31,0x0021,adHi); // addr HI
             BDX_MDIO_WRITE(31,0x0020,adLo); // addr LO
             // get part lenght 
             i++;
             ...
             for(;(pl--) && (i<TN8020_phy_firmware_size);i++){ 
                adHi=(u16) (TN8020_phy_firmware[i]>>16);
                adLo=(u16) TN8020_phy_firmware[i];
                BDX_MDIO_WRITE(31,0x0030,adLo); // data LO
                BDX_MDIO_WRITE(31,0x0030,adHi); // data HI
             }
             if( (i==TN8020_phy_firmware_size) && (++pl) ) {
              ERR("InvalidHPY FW format i=%x aH=%x aL=%x\n",i,adHi,adLo);
             }
         } 
         BDX_MDIO_WRITE(31,0x0000,0); // set PHY reset off
         for(i=100;--i;){
             msleep(10);
             phy_id= bdx_mdio_read(regs, 31, port, 0);
             if(!(phy_id&(1<<1))) break;
         }
         if(!i) ERR("PHY in permanent reset\n");
        }

    do {
         msleep(10);
         BDX_MDIO_WRITE(1,0,0x2040);       // write  1.0 0x2040  # Force 10G                              
    } while (0);
...
static int bdx_mdio_phy_search(void __iomem * regs, int *port_t, unsigned  short *phy_t){
     int i, phy_id;
     char *s;
     i=bdx_mdio_look_for_phy(regs,*port_t);
     if (i<0) return 0; // HPY not found

     *port_t=i;
     phy_id= bdx_mdio_read(regs, 1, *port_t, 0x0002); // PHY_ID_HI
     i=phy_id<<16;
     phy_id= bdx_mdio_read(regs, 1, *port_t, 0x0003); // PHY_ID_LOW
     phy_id &=0xFFFF;
     i |= phy_id;
     ...
     switch(i) {
       case 0x0043A400: 
              *phy_t=PHY_QT2025; 
               s="QT2025 10 Gbps SFP+ or XFP";
               break;
       case 0x03429081: 
               s="AEL2020 10 Gbps SFP+";
               break;
       case 0x00A19410: 
              *phy_t=PHY_TN8020; 
               s="TN8020  10baseT";
     ...
     ERR("HPY detected on port %u ID=%X - %s\n",*port_t,i,s);

     return (PHY_NA == *phy_t)?0:1;
}
#else
#define bdx_mdio_phy_search(regs, port, phy) (0)
#endif

static int bdx_mdio_reset(struct bdx_priv *priv, int port, unsigned  short phy)
{
...
unsigned  short phy_t=phy;
    ...
    if(0==bdx_mdio_phy_search(regs, &port_t, &phy_t)) return 0;
    if(phy!=phy_t) {
     ERR("HPY type by svid %u found %u\n",phy,phy_t);
      if(phy!=PHY_QT2025x){
         phy=phy_t;  
      }
    }
  ...
  ++port;
      ...
      switch(phy) {
      case PHY_QT2025x: // QT2025 10 Gbps XFP
      case PHY_QT2025:  // QT2025 10 Gbps SFP+
                  return  QT2025_mdio_reset(regs,port,phy);

      case PHY_TN8020: // 10BaseT
                  return  TN8020_mdio_reset(regs,port,phy);
      default:
		return 0;
...
static void print_hw_id(struct pci_dev *pdev)
{
	...
	printk(KERN_INFO BDX_DRV_NAME ": %s%s\n", BDX_NIC_NAME,
	       nic->port_num == 1 ? "" : ", 2-Port");
	printk(KERN_INFO
	       BDX_DRV_NAME ": srom 0x%x fpga %d build %u lane# %d"
	       " max_pl 0x%x mrrs 0x%x\n",
	       readl(nic->regs + SROM_VER),
               readl(nic->regs + FPGA_VER) & 0xFFF,
...
{
	printk(KERN_INFO BDX_DRV_NAME ": fw 0x%x\n", readl(nic->regs + FW_VER));
}
...
static int bdx_hw_reset_direct(void __iomem *regs)
{
	...
	ERR(BDX_DRV_NAME ": HW reset failed\n");
	return 1;		/* failure */
}
...
{
	ENTER;
	RET((priv->pdev->device == 0x3009) ? bdx_hw_reset(priv) :
                                             bdx_sw_reset(priv));
}
...
static int bdx_init_rss(struct bdx_priv *priv)
{
    ...
    for (i =0; i < 4 * RSS_HASH_LEN; i += 4) {
            WRITE_REG(priv, regRSS_HASH_BASE + 4 * i, prandom_u32());
    }
...
static int bdx_open(struct net_device *ndev)
{
	...
	bdx_rx_alloc_buffers(priv, priv->rxdb0, &priv->rxf_fifo0);
	bdx_rx_alloc_buffers(priv, priv->rxdb2, &priv->rxf_fifo2);
...
static int bdx_ioctl_priv(struct net_device *ndev, struct ifreq *ifr, int cmd)
{
	...
	switch (data[0]) {
		...
		case 2: data[2] = priv->phy_mdio_port;break;
                default: data[2] = 0xFFFFFFFF; break;
                }
	...
	case OP_MDIO_READ:
                ...
                dev=(u16)(0xFFFF&(data[1]>>16));
		addr=(u16)(0xFFFF&data[1]);
		data[2] =0xFFFF&PHY_MDIO_READ(priv,dev,addr);
		error = copy_to_user(ifr->ifr_data, data, sizeof(data));
		if (error)
			RET(error);
		break;
	...
	case OP_MDIO_WRITE:
                ...
                dev=(u16)(0xFFFF&(data[1]>>16));
		addr=(u16)(0xFFFF&data[1]);
		PHY_MDIO_WRITE(priv,dev,addr,(u16)(data[2]));
		break;
...
static void __bdx_vlan_rx_vid(struct net_device *ndev, uint16_t vid, int enable)
{
	...
	if (unlikely(vid >= 4096)) {
		ERR(BDX_DRV_NAME ": invalid VID: %u (> 4096)\n", vid);
		RET();
	}
...
static int bdx_rx_init(struct bdx_priv *priv)
{
	...
	if (bdx_fifo_init(priv, &priv->rxd_fifo0.m, priv->rxd_size,
		...
		goto err_mem;
	if (bdx_fifo_init(priv, &priv->rxd_fifo2.m, 1,
		      regRXD_CFG0_2, regRXD_CFG1_2,
		      regRXD_RPTR_2, regRXD_WPTR_2))
		goto err_mem;
	if (bdx_fifo_init(priv, &priv->rxf_fifo0.m, priv->rxf_size,
			  regRXF_CFG0_0, regRXF_CFG1_0,
			  regRXF_RPTR_0, regRXF_WPTR_0))
		goto err_mem;
	if (bdx_fifo_init(priv, &priv->rxf_fifo2.m, priv->rxf_size,
		      regRXF_CFG0_2, regRXF_CFG1_2,
		      regRXF_RPTR_2, regRXF_WPTR_2))
		goto err_mem;
	if (!(priv->rxdb0 = bdx_rxdb_create(priv->rxf_fifo0.m.memsz /
					sizeof(struct rxf_desc))))
		goto err_mem;
    /*
     * Note: We only allocate 16 descriptors for rxdb2 to save space. These
     *	     descriptors are needed for some strange reason and AFAIK are not
     *	     used for receiving packets.
     */
	if (!(priv->rxdb2 = bdx_rxdb_create(16)))
		goto err_mem;

	priv->rxf_fifo0.m.pktsz = priv->ndev->mtu + VLAN_ETH_HLEN;
	priv->rxf_fifo2.m.pktsz = priv->ndev->mtu + VLAN_ETH_HLEN;
	return 0;
...
static void bdx_rx_free(struct bdx_priv *priv)
{
	...
	if (priv->rxdb2) {
		bdx_rx_free_buffers(priv, priv->rxdb2, &priv->rxf_fifo2);
		bdx_rxdb_destroy(priv->rxdb2);
		priv->rxdb2 = NULL;
	}
	bdx_fifo_free(priv, &priv->rxf_fifo0.m);
	bdx_fifo_free(priv, &priv->rxf_fifo2.m);
	bdx_fifo_free(priv, &priv->rxd_fifo0.m);
	bdx_fifo_free(priv, &priv->rxd_fifo2.m);
	...
	while (dno > 0) {
                ...
                if (likely(paged_buffers)) {
                        ...
                        if (unlikely(page_off < 0)) {
                                page_size = MIN(LUXOR__MAX_PAGE_SIZE,
                                                dno * buf_size);
                                page_off  = ((page_size / buf_size) - 1) *
                                            buf_size;

                                if ((page = alloc_pages(page_size > PAGE_SIZE ?
                                                        GFP_ATOMIC | __GFP_COMP :
                                                        GFP_ATOMIC,
                                            get_order(page_size))) == NULL) {

                                        ERR("NO MEM: alloc_pages failed\n");
                                        page_size = 0;
                                        break;
                                }
                                dma = pci_map_page(priv->pdev, page,
                                                   0L, page_size,
                                                   PCI_DMA_FROMDEVICE);
                        }
                        else {
                        ...
                        if (!(skb = netdev_alloc_skb(priv->ndev, f->m.pktsz +
                                                     SMP_CACHE_BYTES))) {
		...
		dm	 = bdx_rxdb_addr_elem(db, idx);
                ...
                if (likely(paged_buffers)) {
                        ...
                        skb_reserve(skb, (PTR_ALIGN(skb->data,
                                                    SMP_CACHE_BYTES) -
                                          skb->data));
                        dm->dma = pci_map_single(priv->pdev, skb->data,
                                                 f->m.pktsz,
                                                 PCI_DMA_FROMDEVICE);
                }
	...
	while (size > 0) {
		...
		rxf_fifo = &priv->rxf_fifo0;
                ...
                if (likely(dm->skb == NULL)) {	    /* Handle page descriptor */
                        ...
                        DBG("rxhash    = 0x%x\n", skb->rxhash);
#endif
#if defined(FIX_IP_ID)
                        if (IS_GRO(priv->ndev)) {
                                struct ethhdr *eth = (struct ethhdr *)
                                        ((char *)page_address(dm->page) +
                                         dm->off);
                                ...
                                if (eth->h_proto ==
                                    __constant_htons(ETH_P_IP)) {
                                        struct iphdr *iph = (struct iphdr *)
                                                            (eth + 1);
                                        register u16 id   = ntohs(iph->id);

                                        if (likely(id == priv->id)) {
                                                id += ++(priv->count);
                                                id = htons(id);
...
#endif
                        skb_add_rx_frag(skb, 0, dm->page, dm->off, len , SKB_TRUESIZE(len));
                        bdx_rxdb_free_elem(db, rxdd->va_lo);
                        DBG("SKB: len=%d data_len=%d, truesize=%d head=%p "
                            "data=%p end=%p page=%p page_offset=%d size=%d"
                            "nr_frags=%d\n",
...
err_mem:
	ERR(BDX_DRV_NAME ": %s: Tx init failed\n", priv->ndev->name);
	return -ENOMEM;
}
...
#else
	/*
     * Force memory writes to complete before letting the HW know there are
     * new descriptors to fetch (might be needed on platforms like IA64).
     * wmb();
     */
	WRITE_REG(priv, f->m.reg_WPTR, f->m.wptr & TXF_WPTR_WR_PTR);
...
static void bdx_tx_cleanup(struct bdx_priv *priv)
{
	...
	struct txdb *db = &priv->txdb;
	int tx_level = 0;
        DEF_TIMER(bdx_tx_cleanup);
        START_TIMER(bdx_tx_cleanup);
...
bdx_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	if (!(err = pci_set_dma_mask(pdev, LUXOR__DMA_64BIT_MASK)) &&
		...
		if ((err = pci_set_dma_mask(pdev, LUXOR__DMA_32BIT_MASK)) ||
			...
			printk(KERN_ERR BDX_DRV_NAME ": No usable DMA configuration"
					", aborting\n");
			goto err_dma;
		}
		pci_using_dac = 0;
	...
	if (!pciaddr) {
		err = -EIO;
		ERR(BDX_DRV_NAME ": no MMIO resource\n");
		goto err_out_res;
	}
	if ((regionSize = pci_resource_len(pdev, 0)) < BDX_REGS_SIZE) {
		err = -EIO;
		ERR(BDX_DRV_NAME ": MMIO resource (%x) too small\n", regionSize);
		goto err_out_res;
	}
	...
	if (!nic->regs) {
		err = -EIO;
		ERR(BDX_DRV_NAME ": ioremap failed\n");
		goto err_out_res;
	}

	if (pdev->irq < 2) {
		err = -EIO;
		ERR(BDX_DRV_NAME ": invalid irq (%d)\n", pdev->irq);
		goto err_out_iomap;
	}
	...
	if (bdx_support_msi_by_id(pdev->vendor,pdev->device)){
		if ((err = pci_enable_msi(pdev)))
			ERR(BDX_DRV_NAME ": Can't enable msi. Error is %d\n", err);
		else
	...
	for (port = 0; port < nic->port_num; port++) {
		if (!(ndev = alloc_etherdev(sizeof(struct bdx_priv)))) {
			err = -ENOMEM;
			printk(KERN_ERR BDX_DRV_NAME ": alloc_etherdev failed\n");
			goto err_out_iomap;
		}
		...
		if ((readl(nic->regs + FPGA_VER) & 0xFFF) == 308) {
			...
			priv->stats_flag = 1;
               ...
               /*Init PHY*/
               phy=bdx_get_phy_by_id(pdev->vendor,pdev->device,pdev->subsystem_device,port);
	       bdx_mdio_reset(priv, port, phy);
		...
		if (bdx_read_mac(priv)) {
			printk(KERN_ERR BDX_DRV_NAME ": load MAC address failed\n");
			goto err_out_iomap;
		}
		SET_NETDEV_DEV(ndev, &pdev->dev);
		strcpy(ndev->name, "geth%d");
		if ((err = register_netdev(ndev))) {
			printk(KERN_ERR BDX_DRV_NAME ": register_netdev failed\n");
			goto err_out_free;
		}
 ...
 * @ecmd
 */
static int bdx_get_settings(struct net_device *netdev,
