...
#include "tn40.h"
#include "tn40_fw.h"
#if defined(_TN4022_DEV_)
#include "tn4022_dev.h"
#else
...
#ifdef REG_LOG
u32  bdx_readl(void *reg)
{
    u32 val=readl(reg);
    ERR("READ_REG reg=0x%x val=0x%x \n",(((u32)reg)&0xFFFF) ,val);
    return val;
}
#endif

#ifdef USE_chack_for_hang
void cfh_timer_fn(unsigned long arg)
{
    struct bdx_priv *priv=(struct bdx_priv *)arg;
    u32 link=READ_REG(priv, regMAC_LNK_STAT) & MAC_LINK_STAT;

    if (!link)
    {
        WRITE_REG(priv, 0x104c, 0x4e);
        WRITE_REG(priv, 0x1050, 0x4e);
        WRITE_REG(priv, 0x1054, 0x4e);
        WRITE_REG(priv, 0x1058, 0x4e);
    ...
    if(priv->cfh_is_on)
    {
        priv->cfh_timer.expires +=2*HZ; /*2 sec delay*/
        add_timer(&priv->cfh_timer);
    }
}

static void stop_cfh(struct bdx_priv *priv)
{
    if(priv->cfh_is_on)
    {
        priv->cfh_is_on=0;
        del_timer(&priv->cfh_timer);
    }
}

static void start_cfh(struct bdx_priv *priv)
{
    unsigned long j = jiffies;
    init_timer(&priv->cfh_timer);
    priv->cfh_timer.data = (unsigned long)priv;
    priv->cfh_timer.function = cfh_timer_fn;
    priv->cfh_timer.expires = j + 2*HZ;
    priv->cfh_is_on=1;
    add_timer(&priv->cfh_timer);

}
#else
#define  start_cfh(priv)
#define  stop_cfh(priv)
#endif
...
u32 bdx_mdio_get(void __iomem *regs)
{
 ...
 */
int bdx_mdio_read(void __iomem *regs, int device, int port, u16 addr)
{
    u32 tmp_reg;
    ...
    //  ERR("MDIO_READ: MDIO_ADDR =0x%x \n", (u32)addr);
    /* Write read command */
    writel(MDIO_CMD_STAT_VAL(1, device, port), regs + regMDIO_CMD_STAT);
    //  ERR("MDIO_READ: MDIO_CMD_STAT =0x%x \n", MDIO_CMD_STAT_VAL(1, device, port));
 ...
 */
int bdx_mdio_write(void __iomem *regs, int device, int port, u16 addr, u16 data)
{
    u32 tmp_reg;
    ...
    /* Write address */
    //  ERR("MDIO_WRITE: MDIO_ADDR =0x%x \n", (u32)addr);
    writel((u32)addr, regs + regMDIO_ADDR);
    /* Write data */
    //  ERR("MDIO_WRITE: MDIO_DATA =0x%x \n", (u32)data);
    writel((u32)data, regs + regMDIO_DATA);
    /* Write write command */
    //  ERR("MDIO_WRITE: MDIO_CMD_STAT =0x%x \n", MDIO_CMD_STAT_VAL(0, device, port));
...
int bdx_mdio_look_for_phy(void __iomem *regs, int port)
{
    int i=port, phy_id;
    //     ERR("LOOK FOR HPY: set MDIO_CMD_STAT=0x%x  val=0x%x\n",regMDIO_CMD_STAT,(1<<13));
    writel((1<<13), regs + regMDIO_CMD_STAT);
...
static int bdx_mdio_phy_search(struct bdx_priv *priv, void __iomem * regs, int *port_t, unsigned  short *phy_t)
{
    int i, phy_id;
    char *s;
    ...
    ERR("PHY detected on port %u ID=%X - %s\n",*port_t,i,s);
...
static int bdx_mdio_reset(struct bdx_priv *priv, int port, unsigned  short phy)
{
    ...
    priv->phy_type		= phy;
    return priv->phy_ops.mdio_reset(regs, port, phy);
}
...
static void print_hw_id(struct pci_dev *pdev)
{
    ...
    printk(KERN_INFO
           BDX_DRV_NAME ": srom 0x%x HWver %d build %u lane# %d"
...
void bdx_speed_changed(struct bdx_priv *priv, u32 speed)
{
    ...
    {
        case SPEED_10000:
            priv->link_speed=SPEED_10000;

            if(priv->phy_type==PHY_TYPE_TN8020)
            {
                bdx_mdio_write(priv->pBdxRegs, 1, priv->phy_mdio_port, 0, 0x2040); // write  1.0 0x2040  # Force 10G
            }

            //        BDX_MDIO_WRITE(1,0,0x2040);
        ...
        case SPEED_1000:
            ...
            if(priv->phy_type==PHY_TYPE_TN8020)
            {
                bdx_mdio_write(priv->pBdxRegs, 1, priv->phy_mdio_port, 0, 0x2000); // write  1.0 0x2040  # Force 1G
            }

            //         BDX_MDIO_WRITE(1,0,0x2000);       // write  1.0 0x2000  # Force 1G
        ...
        case SPEED_100:
            if(priv->phy_type==PHY_TYPE_TN8020)
            {
                bdx_mdio_write(priv->pBdxRegs, 1, priv->phy_mdio_port, 0, 0x2000); // write  1.0 0x2040  # Force 100M
            }
            priv->link_speed=SPEED_100;
            WRITE_REG(priv, 0x1010, 0x613); /*ETHSD.REFCLK_CONF  */
            WRITE_REG(priv, 0x104c, 0x4d);  /*ETHSD.L0_RX_PCNT  */
            WRITE_REG(priv, 0x1050, 0x0);  /*ETHSD.L1_RX_PCNT  */
            WRITE_REG(priv, 0x1054, 0x0);  /*ETHSD.L2_RX_PCNT  */
            ...
            WRITE_REG(priv, 0x6350, 0x27); /*MAC.PCS_IF_MODE 100m*/
            WRITE_REG(priv, 0x6310, 0x9501); /*MAC.PCS_DEV_AB */
            WRITE_REG(priv, 0x6314, 0x1); /*MAC.PCS_PART_AB */
            WRITE_REG(priv, 0x6348, 0xc8); /*MAC.PCS_LINK_LO */
            WRITE_REG(priv, 0x634c, 0xc8); /*MAC.PCS_LINK_HI */
            udelay(50);
...
static void bdx_link_changed(struct bdx_priv *priv)
{
    ...
    else
    {
        if (!netif_carrier_ok(priv->ndev))
        {
            ...
            ERR("%s: Link Up %s\n", priv->ndev->name, (link == SPEED_10000) ? "10G"   :
            										  (link == SPEED_1000)  ? "1G"    :
            										  (link == SPEED_100)   ? "100M"  :
            										                          " strange speed");
        }
...
static inline void bdx_isr_extra(struct bdx_priv *priv, u32 isr)
{
    ...
    if (isr & IR_RX_FREE_0)
    {
#if 0 // !defined VM_KLNX
        bdx_rx_alloc_buffers(priv, priv->rxdb0, &priv->rxf_fifo0);
#endif
        DBG("RX_FREE_0\n");
    }

    i=(isr & (IR_LNKCHG0|IR_TMR0));
    //       if (i){       ERR(" (IR_LNKCHG0|IR_TMR0) ISR_MSK0=%8.8X\n",isr); }

    if (isr & (IR_LNKCHG1|IR_TMR0))
    {
        if (priv->phy_type == PHY_TYPE_TN8020)
        {
            u32 v=bdx_mdio_read(priv->pBdxRegs,1,priv->phy_mdio_port,0x9005);
            if(v & 1)
                i = 1;
        }
        else
        ...
        //          ERR("ISR_MSK0=%8.8X PHY 1.9005=%x\n",isr,v);
    }
...
static int bdx_init_rss(struct bdx_priv *priv)
{
    ...
    {
        WRITE_REG(priv, regRSS_HASH_BASE + 4 * i, random32());
    }
    ...
    while (size > 0)
    {
        ...
        if (unlikely(rxd_err = GET_RXD_ERR(rxd_val1)))
        {
            ...
            if (rxd_err == 0x8)     // UDP checksum error
            {
                ...
                struct udphdr *udp = (struct udphdr *)(pkt + (sizeof(struct ethhdr) + ((eth->h_proto == __constant_htons(ETH_P_8021Q)) ? VLAN_HLEN : 0) + sizeof(struct iphdr)));
                if (udp->check == 0)
                {
                    DBG2("false rxd_err = 0x%x\n", rxd_err);
                    bErr = 0;           // Work around H/W false error indication
                }
...
#else
            skb_add_rx_frag(skb, 0, dm->page, dm->off, len);
#endif
               ...
               struct txd_desc *txdd)
{
    ...
    if(skb->len<60)  ++nr_frags;	// SHORT_PKT_FIX
...
static int bdx_tx_transmit(struct sk_buff *skb, struct net_device *ndev)
{
    ...
    if(skb->len<60)
    {
        ...
        pbl->len        = CPU_CHIP_SWAP32(60-skb->len);
...
bdx_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
    ...
    for (port = 0; port < nic->port_num; port++)
    {
        ...
        if (bdx_mdio_reset(priv, port, phy) == -1)
        {
        	err = -ENODEV;
        	goto err_out_iomap;
    	}
