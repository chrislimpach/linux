...
__init int MV88X3120_mdio_reset(struct bdx_priv *priv, int port,  unsigned short phy)
{
    ...
    int             rVal = 0;
    u32             j, MV88X3120_phy_firmware_size=sizeof(MV88X3120_phy_firmware)/sizeof(u16) - MV88X3120_FILE_HEADER_SIZE;
    void __iomem 	*regs=priv->pBdxRegs;

    ENTER;

    val  = bdx_mdio_read(regs, 1, port, 3);
    val  = (val & (MANUF_MODEL_NUM_MASK << MANUF_MODEL_NUM_BIT_POS)) >> MANUF_MODEL_NUM_BIT_POS; // modelNo
    val1 = bdx_mdio_read(regs, 3, port, 53249);                                                  // revNo
    if(val == 8 && val1 < 3)
    {
        val = 9;
    }
    ...
    do
    {
        val = bdx_mdio_read(regs, 1, port, 49152);
        BDX_MDIO_WRITE(1,49152, (val | 1<<15));                         // Writes a self clearing reset
        msleep(250);                                                    // Allow reset to complete
        // Make sure we can access the DSP
        // and it's in the correct mode (waiting for download)
        if ((val = bdx_mdio_read(regs, 3, port, 0xD000)) != 0x000A)
        {
        ...
        BDX_MDIO_WRITE(3,0xD004,0);                                     // Set starting address in RAM to 0x0000 */
        BDX_MDIO_WRITE(3,0xD005,0);
        for (j = MV88X3120_FILE_HEADER_SIZE; j < MV88X3120_phy_firmware_size; j++)
        {
            val = swab16(MV88X3120_phy_firmware[j]);
            BDX_MDIO_WRITE(3, 0xD006, val);
        }
        DBG("MV88X3120 loaded %d 16bit words\n", j);

        //val = bdx_mdio_read(regs, 7, port, 32);
        //BDX_MDIO_WRITE(3,0xD000,(val & (~(1<<13))));                  // Single port
        val = bdx_mdio_read(regs, 3, port, 0xD000);
        BDX_MDIO_WRITE(3,0xD000,(val | (1<<6) ));                       // Code uploaded
        msleep(500);                                                    // Give firmware code time to start
        // make sure the firmware code started
        if (!((val = bdx_mdio_read(regs,3, port, 0xD000)) & (1<<4)))    // Application started
        {
        ...
        for (j = 0; j < 10; j++)
        {
            if (!((val = bdx_mdio_read(regs,1, port, 0)) & (1<<15)))
            {
                break;
            }
        ...
        BDX_MDIO_WRITE(4,49152, 0x00c2);                                // Change Tx/Rx polarity
        //      BDX_MDIO_WRITE(7,60, 0);                                        // Disable EEE
        BDX_MDIO_WRITE(1,36866, 0x0009);                                // Enable LASI auto negotiation complete
        val  = bdx_mdio_read(regs, 1, port, 49190);
        val1 = bdx_mdio_read(regs, 1, port, 49191);
        ERR("MV88X3120 FW version is %d.%d.%d.%d\n",((val & 0xff00) >> 8), (val & 0x00ff), ((val1 & 0xff00) >> 8), (val1 & 0x00ff));
        val = bdx_mdio_read(regs, 7, port, 1);
...
static int  MV88X3120_get_link_speed(struct bdx_priv *priv)
{
    ...
    ENTER;
#ifdef DEBUG
    val = bdx_mdio_read(priv->pBdxRegs, 3, priv->phy_mdio_port, 8);
    val = bdx_mdio_read(priv->pBdxRegs, 3, priv->phy_mdio_port, 8);
    DBG("MV88X3120 3.8 = 0x%x\n", val);
    val = bdx_mdio_read(priv->pBdxRegs, 3, priv->phy_mdio_port, 1);
    val = bdx_mdio_read(priv->pBdxRegs, 3, priv->phy_mdio_port, 1);
    DBG("MV88X3120 3.1 = 0x%x\n", val);
    val = bdx_mdio_read(priv->pBdxRegs, 1, priv->phy_mdio_port, 49192);
    val = bdx_mdio_read(priv->pBdxRegs, 1, priv->phy_mdio_port, 49192);
    DBG("MV88X3120 1.49192 = 0x%x\n", val);
#endif
    val = bdx_mdio_read(priv->pBdxRegs, 1, priv->phy_mdio_port, 36869);
    val = bdx_mdio_read(priv->pBdxRegs, 1, priv->phy_mdio_port, 36869);
    DBG("MV88X3120 1.36869 = 0x%x\n", val);
    val = bdx_mdio_read(priv->pBdxRegs, 7, priv->phy_mdio_port, 1);
    val = bdx_mdio_read(priv->pBdxRegs, 7, priv->phy_mdio_port, 1);
    DBG("MV88X3120 7.1 = 0x%x\n", val);
    if (val & (1 << 2))                                         // Link up
    {
        leds = bdx_mdio_read(priv->pBdxRegs, 1, priv->phy_mdio_port, 49161);
        val  = bdx_mdio_read(priv->pBdxRegs, 1, priv->phy_mdio_port, 49192);
        val  = (val & 0x00F0) >> 4;
        switch(val)
        {
            case SPEED_RES_10GIG     :
                link = SPEED_10000;
                bdx_mdio_write(priv->pBdxRegs, 1, priv->phy_mdio_port, 49161, (leds &0xfff0) | 0x0009); // led_speed0 on
                //ERR("MV88X3120 10G link detected\n");
                break;

            case SPEED_RES_1GIG_FD   :
                link = SPEED_1000;
                bdx_mdio_write(priv->pBdxRegs, 1, priv->phy_mdio_port, 49161, (leds &0xfff0) | 0x0006); // led_speed1 on
                ERR("MV88X3120 1G link detected\n");
                break;

            case SPEED_RES_100M_FD   :
            case SPEED_RES_100M_HD   :
                link = SPEED_100;
                bdx_mdio_write(priv->pBdxRegs, 1, priv->phy_mdio_port, 49161, (leds &0xfff0) | 0x000a); // led_speeds off
                ERR("MV88X3120 100M %s link detected\n", (val == SPEED_RES_100M_HD) ? "Half Duplex" : "");
                break;

            case AN_IN_PROGRESS:
                bdx_mdio_write(priv->pBdxRegs, 1, priv->phy_mdio_port, 49161, (leds &0xfff0) | 0x000a); // led_speeds off
                ERR("MV88X3120 auto negotiation in progress...\n");
                break;

            default:
                bdx_mdio_write(priv->pBdxRegs, 1, priv->phy_mdio_port, 49161, (leds &0xfff0) | 0x000a); // led_speeds off
                ERR("MV88X3120 internal error - unknown link speed value (0x%x) !\n", val);
                break;
        }
        priv->errmsg_count = 0;
    }
...
u32 MV88X3120_link_changed(struct bdx_priv *priv)
{
	...
	{
		u32 timeout;
		u16 leds = bdx_mdio_read(priv->pBdxRegs, 1, priv->phy_mdio_port, 49161);
		bdx_mdio_write(priv->pBdxRegs, 1, priv->phy_mdio_port, 49161, (leds &0xfff0) | 0x000a); // led_speeds off
		if (speed)
		{
			...
			if(priv->link_loop_cnt++ > LINK_LOOP_MAX)
			{
				...
				ERR("MV88X3120 trying to recover link after %d tries\n", LINK_LOOP_MAX);
			}
		}
		else
		{
...
void MV88X3120_leds(struct bdx_priv *priv, enum PHY_LEDS_OP op)
{
	...
	{
		case PHY_LEDS_SAVE:
			priv->phy_ops.leds = bdx_mdio_read(priv->pBdxRegs, 1, priv->phy_mdio_port, 49161);
			bdx_mdio_write(priv->pBdxRegs, 1, priv->phy_mdio_port, 49161, 0xf);
			bdx_mdio_write(priv->pBdxRegs, 1, priv->phy_mdio_port, 49161, 0xff);
			break;

		case PHY_LEDS_RESTORE:
			bdx_mdio_write(priv->pBdxRegs, 1, priv->phy_mdio_port, 49161, priv->phy_ops.leds);
			break;
