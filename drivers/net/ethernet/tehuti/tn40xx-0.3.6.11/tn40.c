...
int bdx_mdio_write(void __iomem *regs, int device, int port, u16 addr, u16 data)
{
	...
	writel((u32)addr, regs + regMDIO_ADDR);
	if (bdx_mdio_get(regs)== 0xFFFFFFFF) {		return -1;	}
...
static int __init bdx_mdio_phy_search(struct bdx_priv *priv, void __iomem * regs, int *port_t, unsigned  short *phy_t)
{
    ...
    ERR("%s: PHY detected on port %u ID=%X - %s\n",BDX_DRV_NAME, *port_t,i,s);
...
void bdx_speed_changed(struct bdx_priv *priv, u32 speed)
{
    ...
    switch(speed)
    {
        ...
        case 0: // Link down
            ...
            break;
        default:
            ERR("%s: %s Link speed was not identified yet (%d)\n", BDX_DRV_NAME, priv->ndev->name, speed);
            priv->link_speed=0;
    }
...
static void bdx_link_changed(struct bdx_priv *priv)
{
    ...
    else
    {
        if (!netif_carrier_ok(priv->ndev))
        {
            ...
            ERR("%s: %s Link Up %s\n", BDX_DRV_NAME, priv->ndev->name, (link == SPEED_10000) ? "10G"   :
            										  (link == SPEED_1000)  ? "1G"    :
            										  (link == SPEED_100)   ? "100M"  :
            										                          " strange speed");
        }
...
static inline void bdx_isr_extra(struct bdx_priv *priv, u32 isr)
{
    ...
    if (isr & IR_PCIE_LINK)
        ERR("%s: %s PCI-E Link Fault\n", BDX_DRV_NAME, priv->ndev->name);

    if (isr & IR_PCIE_TOUT)
        ERR("%s: %s PCI-E Time Out\n", BDX_DRV_NAME, priv->ndev->name);
#endif
}
...
static int __init bdx_fw_load(struct bdx_priv *priv)
{
    ...
    if (i == 200)
    {
        ERR("%s: %s firmware loading failed\n", BDX_DRV_NAME, priv->ndev->name);
    ...
    {
        DBG("%s: %s firmware loading success\n", BDX_DRV_NAME, priv->ndev->name);
    }
...
static int bdx_sw_reset(struct bdx_priv *priv)
{
    ...
    if (i == 50)
        ERR("%s: %s SW reset timeout. continuing anyway\n",
        		BDX_DRV_NAME, priv->ndev->name);
...
bdx_vlan_rx_register(struct net_device *ndev, struct vlan_group *grp)
{
    ...
    ENTER;
    DBG("%s: device ='%s', group ='%p'\n", BDX_DRV_NAME, ndev->name, grp);
    priv->vlgrp = grp;
    RET();
}
#endif
    ...
    while (dno > 0)
    {
        ...
        if (likely(paged_buffers))
        {
            ...
            if (unlikely(page_off < 0))
            {
                ...
                {
                    //WRN("%s : warning - system memory is temporary low\n", BDX_DRV_NAME);
                    page_size = 0;
                    break;
                }
...
static int bdx_rx_receive(struct bdx_priv *priv, struct rxd_fifo *f, int budget)
{
    ...
    while (size > 0)
    {
        ...
        if (unlikely(rxd_err = GET_RXD_ERR(rxd_val1)))
        {
            ...
            else if ((rxd_err == 0x10) && (len == 60))     	// TCP checksum error
            {
            	u16 tcpSum;
                char *pkt = ((char *)page_address(dm->page) +  dm->off);
                struct ethhdr *eth = (struct ethhdr *)pkt;
...
err_mem:
    ERR(BDX_DRV_NAME ": %s: %s Tx init failed\n", BDX_DRV_NAME, priv->ndev->name);
    return -ENOMEM;
}
...
static void bdx_blink_callback(unsigned long data)
{
    ...
    priv->phy_ops.leds = !priv->phy_ops.leds;
    if (priv->phy_ops.leds)
    {
    	priv->phy_ops.ledset(priv, PHY_LEDS_ON);
    }
    else
...
static int bdx_hotplug_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	ERR(BDX_DRV_NAME ": rescan/hotplug is *NOT* supported!, please use rmmod/insmod instead\n");
	RET(-1);
...
static void __init bdx_no_hotplug(void)
{
	...
	spin_unlock(&g_lock);
	ERR(BDX_DRV_NAME ": detected %d cards, %d loaded\n", nDevices, nLoaded);
