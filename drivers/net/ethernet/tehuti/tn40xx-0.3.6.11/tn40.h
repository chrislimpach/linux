...
#define MSG(fmt, args...)  printk(KERN_ERR fmt, ## args)
...
#define DBG(fmt, args...)   if (g_dbg)printk(KERN_ERR  "%s:%-5d: " fmt, __func__, __LINE__, ## args)
#else
#define DBG_ON
#define DBG_OFF
...
#ifdef WARNING
#define WRN(fmt, args...)   if (g_dbg)printk(KERN_ERR  "%s:%-5d: " fmt, __func__, __LINE__, ## args)
#else
#define WRN(fmt, args...)
#endif
...
#define ENTER        do { if (g_ftrace) printk(KERN_ERR  "%s: - enter\n" , __func__); } while (0)
#define RET(args...) do { if (g_ftrace) printk(KERN_ERR  "%s: - return\n", __func__); return args; } while (0)
#else
#define FTRACE_ON
#define FTRACE_OFF
#define ENTER
...
enum
{
    OP_INFO,OP_READ_REG,OP_WRITE_REG,OP_MDIO_READ,OP_MDIO_WRITE,OP_LED_ON,OP_LED_OFF
};
...
#else
#define regMDIO_CMD_STAT    0x0304
#define regMDIO_DATA        0x0308
#define regMDIO_ADDR        0x030C
#define GET_MDIO_BUSY(x)    GET_BITS_SHIFT(x, 1, 0)
#define GET_MDIO_RD_ERR(x)  GET_BITS_SHIFT(x, 1, 1)
#define MDIO_CMD_STAT_VAL(rd_wrn, dev_addr, port_addr) \
    ((1<<13) | ((rd_wrn) << 2) | ((dev_addr) << 3) | ((port_addr) << 8))
#endif
...
#if defined(RHEL_RELEASE_CODE) && (RHEL_RELEASE_CODE >= 1285)
