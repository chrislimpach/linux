...
#define PHY_NAME					((priv->pci_device == 0x4027) ? "MV88X3310" : "MV88E2010")
enum PHY_LED_COLOR
{
	PHY_LED_OFF,
    PHY_LED_GREEN,
...
static int  MV88X3310_set_Reset_values(struct bdx_priv *priv)
{
	...
	BDX_MDIO_WRITE(priv, 31, 0xF000, 0x78C8);
...
u32 MV88X3310_link_changed(struct bdx_priv *priv)
{
	...
	if(speed != (u32)priv->link_speed)
	{
		switch (speed)
		{
			...
			break;
			case SPEED_1000:
				MSG("%s 1G link detected\n", PHY_NAME);
				break;
			case SPEED_1000X:
				MSG("%s 1GX link detected\n", PHY_NAME);
				break;
			case SPEED_5000:
				MSG("%s 5G link detected\n", PHY_NAME);
				break;
			case SPEED_2500:
				MSG("%s 2.5G link detected\n", PHY_NAME);
				break;
			case SPEED_100:
				MSG("%s 100M link detected\n", PHY_NAME);
				break;
			case SPEED_100X:
				MSG("%s 100MX link detected\n", PHY_NAME);
				break;
			default:
				MSG("%s link down.\n", PHY_NAME);
				break;
