...
static struct bdx_device_descr  bdx_dev_tbl[] =
{
	...
	LDEV(TEHUTI_VID,0x4025,0x8102,1,1,AQR105,NA,	"Edimax EN-8102P 10GbE Ethernet Adapter"),
	LDEV(TEHUTI_VID,0x4025,0x7203,1,1,AQR105,NA,	"Promise Technology SANLink3 NBase-T1"),
#endif
	{0}
};
...
static int __init bdx_mdio_phy_search(struct bdx_priv *priv, void __iomem * regs, int *port_t, unsigned  short *phy_t)
{
	...
	switch(i)
    {
        ...
        case 0x02b09aa:
        	if (priv->pci_device == 0x4027)
        	{
        		s="MV88X3310 (A0) 10Gbps 10GBase-T";
        		*phy_t = PHY_TYPE_MV88X3310;
        	}
        	else if (priv->pdev->device == 0x4527)
        	{
        		s="MV88E2010 (A0) 5Gbps 5GBase-T";
        		*phy_t = PHY_TYPE_MV88E2010;
        	}
        	else
        	{
        		s = "";
    ...
    else
    {
        if (!netif_carrier_ok(priv->ndev))
        {
            ...
            MSG("%s Link Up %s\n", priv->ndev->name, (priv->link_speed == SPEED_10000) ? "10G"   :
            										 ...
            										 (priv->link_speed == SPEED_1000)  ? "1G"    :
            										 (priv->link_speed == SPEED_100)   ? "100M"  :
            										                          " ");
        }
    }
...
static int bdx_set_settings(struct net_device *netdev, struct ethtool_cmd *ecmd)
{
	struct bdx_priv *priv = netdev_priv(netdev);
	DBG("ecmd->cmd=%x\n", ecmd->cmd);
//    if(NULL==priv->phy_ops.set_settings) return -EPERM;
